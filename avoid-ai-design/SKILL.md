...
description: 'Audit and rewrite frontend UI to remove generic AI design patterns ("AI slop"). Use this skill when asked to "de-slop a UI", "make a design look less AI-generated", "audit a component or page for AI design tells", "fix AI-generated frontend that looks generic", or in Turkish: "tasarımı düzelt", "ai sloptan temizle", "arayüzü denetle", "frontend kodunu düzelt", "tasarımı iyileştir", "tasarım denetimi yap". Covers HTML/CSS and React/Tailwind/shadcn. Supports a detection-only mode that flags patterns without rewriting.'
version: 0.2.0
license: MIT
compatibility: Any AI coding assistant that supports the agentskills.io SKILL.md format (Pi, Claude Code, Cursor, Codex CLI, etc.). No external tools or APIs required; uses a screenshot tool if one is available.
metadata:
  author: ungspirit

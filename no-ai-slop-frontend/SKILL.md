...
## How to use
- **Consult TELLS.md:** Use the `read` tool to load `TELLS.md` located in this skill's directory (`dirname(SKILL.md)/TELLS.md`). It contains the comprehensive master catalog of ~100 tells across 9 categories.
- Generating UI -> apply Layer 1 always + the active Layer 2 style + verify against `TELLS.md`.
- Reviewing UI -> scan against Layer 1 and `TELLS.md`, and fix every hit (use the self-check).
...
## Reference
Always load `TELLS.md` from this skill's directory (`dirname(SKILL.md)/TELLS.md`) using the `read` tool for the complete catalog of ~100 tells with detailed explanations and fixes across typography, color, layout, components, imagery, icons, copy, and motion.
